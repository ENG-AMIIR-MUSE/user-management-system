...
export default function Update() {
  ...
  useEffect(() => {
    const getData = async () => {
      const { data } = await axios.get(`/api/complainments/${params.id}`);
      setName(data.name);
      setEmail(data.email);
  ...
  const handleSm = async (e) => {
    ...
    toast.success("Complainment Updated Successfully..");
    nav("/");
  };
  return (
